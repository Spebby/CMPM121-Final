...
public class AnimateDamage : MonoBehaviour {
    ...
    void Start() {
        ...
        this.GetComponent<TMP_Text>().text = dmg;
    ...
    void Update() {
        ...
        if (progress <= 1) {
            ...
            this.GetComponent<TMP_Text>().fontSize = Mathf.RoundToInt(progress * fontF + (1 - progress) * fontI);
            this.GetComponent<TMP_Text>().fontMaterial.color = Color.Lerp(colorI, colorF, progress);
        }
